            ...
            New-UDCard -Content {
                New-UDCollapsible -Items {
                    ...
                    New-UDCollapsibleItem -Title $title -Icon check -Content {
                        New-UDStaticTable -Headers @('test1 ','test2 ') -Title $title -Content {
...
$GetFiles = New-UDEndpoint -Schedule $Schedule -Endpoint {
    ...
    Foreach ($XMLFile in $XMLFiles){
        ...
        #If ($Cache:PageContent.ContainsKey()
        $Cache:Filenames += New-Object psobject -Property @{
            ...
            Link = New-UDLink -Text $Filename -Url "/Pester/$ID"
        }
...
$page = New-UDPage -Name 'Home' -Content {
    New-UDCard -Content {
        New-UDGrid -Title 'Select Pester' -Headers @('Name', 'Successful Tests','Failed Tests', 'Fixture Count') -Properties @('Link','Successful','Failures','FixtureCount') -Endpoint {
            $Cache:Filenames | ForEach-Object { $_ | Select-Object Successful, FixtureCount, Failures, Link
            } | Out-UDGridData 
        }
    }
}
...
$DynamPage = New-UDPage -Url "/Pester/:ID" -Endpoint {
    Param ($ID)
    $Cache:PageContent.item($ID)
} 
$pages += $DynamPage 
